...
def train_chatbot(intents):
    questions, tags = preprocess_data(intents)
...
def get_response(user_input, vectorizer, label_encoder, model, intents):
...
def main():
    ...
    vectorizer, label_encoder, model = train_chatbot(intents)
    ...
    while True:
        ...
        response = get_response(user_input, vectorizer, label_encoder, model, intents)
        print("Bot:", response)

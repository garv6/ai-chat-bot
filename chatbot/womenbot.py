...
file_path = 'women.json'

def load_intents_women(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        intents = json.load(f)
    return intents

# Preprocess the data
def preprocess_data_women(intents):
    questions = []
    tags = []
...
def train_chatbot_women(intents):
    questions, tags = preprocess_data_women(intents)
...
def get_response_women(user_input, vectorizer, label_encoder, model, intents):
...
# Main function
def main():
    intents = load_intents_women(file_path)
    vectorizer, label_encoder, model = train_chatbot_women(intents)
    ...
    while True:
        ...
        response = get_response_women(user_input, vectorizer, label_encoder, model, intents)
        print("Bot:", response)
